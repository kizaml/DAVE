...
function [Li_Ion_Battery_Stored_Energy,Li_Ion_Battery_Power,Battery_Energy_Stored_Net_Charge_Discharge,Renewable_Charged_Power,Grid_Charged_Power]=Li_Ion_Battery(Strategy,...
    Battery_Energy_Capacity,Battery_Power_Output,Grid_failure_flag_mat,...
...
Li_Ion_Battery_Stored_Energy=zeros(35038+1,1,Stochastic_Iterations);
Li_Ion_Battery_Stored_Energy(1,1,:)=Battery_initial_Charge_Percent*Battery_Energy_Capacity;

Battery_Energy_Stored_Net_Charge_Discharge=zeros(35038,1,Stochastic_Iterations);
...
Battery_Energy_Floor=Battery_floor_percentage*Battery_Energy_Capacity; % MWh
Battery_Energy_Floor_Mat=[0, 0, Battery_Energy_Floor, Battery_Energy_Floor, 0, 0];

% Create  Critical Load 
% Critical_load=8;  % MW
Critical_load_mat=[Battery_Power_Output, Critical_load, Battery_Power_Output, Critical_load, Battery_Power_Output, Critical_load];
...
Dischargeable_Energy_Generation_Deficit_JAN_to_DEC(Dischargeable_Energy_Generation_Deficit_JAN_to_DEC>Battery_Power_Output/4)=Battery_Power_Output/4;
...
%Chargeable_Energy_Generation_Surplus_JAN_to_DEC=Energy_Generation_Surplus_JAN_to_DEC.*(1-Grid_failure_flag_mat(:,1,Iter))+Energy_Generation_Surplus_JAN_to_DEC_Critical.*Grid_failure_flag_mat(:,1,Iter);
...
Chargeable_Energy_Combined(Chargeable_Energy_Combined>Battery_Power_Output/4)=Battery_Power_Output/4;
...
%     Critical_load_mat(Strategy)/(4*Battery_Discharge_Efficiency))   =Critical_load_mat(Strategy)/4;
    ...
    for time_block=1:35038
        ...
        % block
        temp_Battery_Stored_Energy=Li_Ion_Battery_Stored_Energy(time_block,1,Iter)*(1-Battery_Self_Discharge_rate)+...
            Chargeable_Energy_Combined(time_block,1,Iter)*Battery_Charge_Efficiency-...
            Dischargeable_Energy_Generation_Deficit_JAN_to_DEC(time_block,1)/Battery_Discharge_Efficiency;
        % MWh
        
        % Limit the energy able to be stored to the max battery capacity
        temp_Battery_Stored_Energy(temp_Battery_Stored_Energy>Battery_Energy_Capacity)=Battery_Energy_Capacity;
        % MWh
        ...
        % the value
        Temp_Charged_Energy(temp_Battery_Stored_Energy>=Li_Ion_Battery_Stored_Energy(time_block,1,Iter))=temp_Battery_Stored_Energy-Li_Ion_Battery_Stored_Energy(time_block,1,Iter);
        Temp_Charged_Energy(temp_Battery_Stored_Energy<Li_Ion_Battery_Stored_Energy(time_block,1,Iter))=0;

        Temp_Renewable_Charged_Energy=min([Temp_Charged_Energy,min([Chargeable_Energy_Generation_Surplus_JAN_to_DEC(time_block,1,Iter),Battery_Power_Output/4])]);
        ...
        % Store the result
        Li_Ion_Battery_Stored_Energy(time_block+1,1,Iter)=temp_Battery_Stored_Energy; % MWh
    end 
    
% Find the change in Battery stored to determine the true charge/discharge
% rates
% Charge is positive, Discharge is negative
Battery_Energy_Stored_Net_Charge_Discharge(:,1,Iter)=Li_Ion_Battery_Stored_Energy(2:end,1,Iter)-Li_Ion_Battery_Stored_Energy(1:end-1,1,Iter);
Batttery_Energy_Impact_On_Grid(:,1,Iter)=Battery_Energy_Stored_Net_Charge_Discharge(:,1,Iter)*(1+Battery_Self_Discharge_rate);
% MWh 
...
% Efficiencies
Batttery_Energy_Impact_On_Grid(Batttery_Energy_Impact_On_Grid>0)=Batttery_Energy_Impact_On_Grid(Batttery_Energy_Impact_On_Grid>0)/Battery_Charge_Efficiency;
Batttery_Energy_Impact_On_Grid(Batttery_Energy_Impact_On_Grid<0)=Batttery_Energy_Impact_On_Grid(Batttery_Energy_Impact_On_Grid<0)*Battery_Discharge_Efficiency;
% MWh

Renewable_Charged_Energy_Impact_on_Grid=Renewable_Charged_Energy/Battery_Charge_Efficiency;
Grid_Charged_Energy_Impact_on_Grid=Grid_Charged_Energy/Battery_Charge_Efficiency;
% MWh

Renewable_Charged_Power=Renewable_Charged_Energy_Impact_on_Grid*4;
Grid_Charged_Power=Grid_Charged_Energy_Impact_on_Grid*4;
% MW



Li_Ion_Battery_Power=Batttery_Energy_Impact_On_Grid*4;
% MW
% Where (+ is charging) 
...
%%
%
%plot(Battery_Power_Impact_On_Grid(1:5*24*4,1,1))
%%
% 
